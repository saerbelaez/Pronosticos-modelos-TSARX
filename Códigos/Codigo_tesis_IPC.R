...
###################### Extrae el valor umbral        
#####################################     tar3e.thresxG        Metropolis-Hasting, con densidad instrumental GAUSSIANA , caso tres regímenes
...
####IPC - Spread en diferencia anual
######################## CARGUE DE BASES DE DATOS

setwd("D:/sarbelaez/Documents/Maestría/Tesis/Datos finales")
Datos_IPC=read.xlsx("IPC_serie.xlsx",startRow = 13)
Datos_IPC=Datos_IPC[606:774,]
IPC=ts(Datos_IPC$Índice,start = c(2004,12),freq=12)
par(mfrow=c(1,1))
plot(IPC,
     main = "Índice de Precios al Consumidor mensual Colombia 2004-2019",
     xlab = "Tiempo",
     ylab = "Índice de Precios al Consumidor")

adf.test(IPC)
ndiffs(IPC)
diff(IPC,lag=1)
Xt=diff(log(IPC),lag=1)*100
...
TIB_mensual=TIB_mensual[107:286,]
...
Spread=ts(Datos$spread,start = 2004,freq=12)


Zt=diff(Spread,lag=12)
plot(Zt)
Zt=ts(Zt,start=c(2005,1),freq=12)
...
h=25
s=12
step.thv=0.132  #se ajusta el tamaño del paso de acuerdo con los resultados (tasa de aceptación)
d0=2
...
par(mfrow=c(2,1))
acf(res, main="",lag.max=50)        # función de autocorrelación muestral
pacf(res, main="",ylab="PACF",lag.max=50)       # función de autocorrelación parcial muestral

resc=res^2 #residuales al cuadrado
par(mfrow=c(2,1))
acf(resc, main="(a)",xlab="",ylab="",lag.max=50)
pacf(resc, main="(b)",xlab="",ylab="",lag.max=50)
...
co=0.13641####Valor del cuantil aproximado para cusumsq para n/2
...
Modelo.zt=forecast::Arima(Zt,order=c(2,0,0),include.constant = F,fixed=c(NA,NA))
...
pronosticos<-function(LH, x, thresVar, Iteration, Burnin, step.thv, d0)
{
  ...
  for (i in 1:NN)
  {
    ...
    for (hh in 1:LH)
    {
      zv[nx+hh]=1.1898*zv[nx+hh-1]-0.2875*zv[nx+hh-2]+rnorm(1, mean=0, sd=sqrt(0.6148))   ## SE CAMBIA LA FORMULA, el kernel de transici?n, variable umbral
    }
...
M1=read.csv2("M1.csv",sep=";")
M1=ts(M1$Miles.de.millones.COP[252:420],start=c(2004,12),freq=12)
Zt=diff(log(M1))*100
####################################### METODO DE CONGDON
set.seed(525)
repeticiones<-1     #### número de repeticiones
...
#### para desempleo, son maximos p=1,P=1 y q=3. d0=3
set.seed(525)
lagp1.1<-1:2
lagP1.1<-1:2
lagq1.1<-1
...
forecast::Acf(Zt, main="",xlab="Tiempo",ylab="ISE original",lag.max=50)        # función de autocorrelación muestral
forecast::Pacf(Zt, main="",xlab="Tiempo",ylab="ISE original",lag.max=50)       # función de autocorrelación parcial muestral
Modelo.zt=forecast::Arima(Zt,order=c(1,0,0),include.constant = F)
lmtest::coeftest(Modelo.zt)
...
tar.summarypron=function(x) 
{
  ...
  for (i in 1:h) 
  {
    ...
    temp[i, 2] <- median(x[, i])
    temp[i, 3] <- sd(x[, i])
    temp[i, 4:5] <- quantile(x[, i], c(0.05, 0.95))
    temp[i, 6:7] <- quantile(x[, i], c(0.025, 0.975))
...
pronosticos<-function(LH, x, thresVar, Iteration, Burnin, step.thv, d0)
{
  ...
  out2<-BAYES2E.TARX(x, lagp1, lagp2, lagP1, lagP2,  lagq1, lagq2, Iteration, Burnin, step.thv, h, constant, thresVar, s, d0)
  ...
  if (constant==1)
  {
    conppron[[1]]=salida2[,1]
    parppron[[1]]=salida2[,2:(p1+constant)]
    parPpron[[1]]=salida2[,(p1+constant+1):(p1+constant+P1)]
    parqpron[[1]]=salida2[,(p1+constant+P1+1):(p1+constant+P1+q1)]
    conppron[[2]]=salida2[,p1+constant+P1+q1+1]
    parppron[[2]]=salida2[,(p1+constant+P1+q1+1+1):(p1+constant+P1+q1+p2+constant)]
    parPpron[[2]]=salida2[,(p1+constant+P1+q1+p2+constant+1):(p1+constant+P1+q1+p2+constant+P2)]
    parqpron[[2]]=salida2[,(p1+constant+P1+q1+p2+constant+P2+1):(p1+constant+P1+q1+p2+constant+P2+q2)]
    
    sigpron[[1]]=salida2[,p1+constant+P1+q1+p2+constant+P2+q2+1]
    sigpron[[2]]=salida2[,p1+constant+P1+q1+p2+constant+P2+q2+2]
    
    thres.1=salida2[,p1+constant+P1+q1+p2+constant+P2+q2+3]
    
    lagd=salida2[,p1+constant+P1+q1+p2+constant+P2+q2+4]
  ...
  {
    conppron[[1]]=matrix(0,nrow=(Iteration-Burnin),ncol=1)
    parppron[[1]]=salida2[,1:p1]
    parPpron[[1]]=salida2[,(p1+1):(p1+P1)]
    parqpron[[1]]=salida2[,(p1+P1+1):(p1+P1+q1)]
    conppron[[2]]=matrix(0,nrow=(Iteration-Burnin),ncol=1)
    parppron[[2]]=salida2[,(p1+P1+q1+1):(p1+P1+q1+p2)]
    parPpron[[2]]=salida2[,(p1+P1+q1+p2+1):(p1+P1+q1+p2+P2)]
    parqpron[[2]]=salida2[,(p1+P1+q1+p2+P2+1):(p1+P1+q1+p2+P2+q2)]
    
    sigpron[[1]]=salida2[,p1+P1+q1+p2+P2+q2+1]
    sigpron[[2]]=salida2[,p1+P1+q1+p2+P2+q2+2]
    
    thres.1=salida2[,p1+P1+q1+p2+P2+q2+3]
    
    lagd=salida2[,p1+P1+q1+p2+P2+q2+4]
  ...
  for (i in 1:NN)
  {
    ...
    for (hh in 1:LH)
    {
      zv[nx+hh]=-0.1829*zv[nx+hh-1]+rnorm(1, mean=0, sd=sqrt(25.39))   ## SE CAMBIA LA FORMULA, el kernel de transici?n, variable umbral
    }
...
for (i in 0:14) {
  set.seed(525)
  Datos_IPC=read.xlsx("IPC_serie.xlsx",startRow = 13)
  Datos_IPC=Datos_IPC[606:(774+(i)),]
  IPC=ts(Datos_IPC$Índice,start = c(2004,12),freq=12)
  Xt=diff(log(IPC),lag=1)*100
  M1=read.csv2("M1.csv",sep=";")
  M1=ts(M1$Miles.de.millones.COP[252:(420+(i))],start=c(2004,12),freq=12)
  Zt=diff(log(M1))*100
  ...
  px=p$seriex
  pz=p$seriez
  AA[i+1,]= t(tar.summarypron(px))[1,]
}


Pronostico=ts(AA[,1],start(2019,1),freq=12)
Datos_IPC=read.xlsx("IPC_serie.xlsx",startRow = 13)
Datos_IPC=Datos_IPC[774:789,]
IPC=ts(Datos_IPC$Índice,start = c(2019,1),freq=12)
Comp_des=diff(log(IPC),lag=1)*100
par(mfrow=c(1,1))
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_1_IPC_M1.xlsx",overwrite = T)
...
for (i in 0:7) {
  set.seed(525)
  Datos_IPC=read.xlsx("IPC_serie.xlsx",startRow = 13)
  Datos_IPC=Datos_IPC[606:(774+(2*i)),]
  IPC=ts(Datos_IPC$Índice,start = c(2004,12),freq=12)
  Xt=diff(log(IPC),lag=1)*100
  M1=read.csv2("M1.csv",sep=";")
  M1=ts(M1$Miles.de.millones.COP[252:(420+(2*i))],start=c(2004,12),freq=12)
  Zt=diff(log(M1))*100
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_2_IPC_M1.xlsx",overwrite = T)
...
for (i in 0:4) {
  set.seed(525)
  Datos_IPC=read.xlsx("IPC_serie.xlsx",startRow = 13)
  Datos_IPC=Datos_IPC[606:(774+(3*i)),]
  IPC=ts(Datos_IPC$Índice,start = c(2004,12),freq=12)
  Xt=diff(log(IPC),lag=1)*100
  M1=read.csv2("M1.csv",sep=";")
  M1=ts(M1$Miles.de.millones.COP[252:(420+(3*i))],start=c(2004,12),freq=12)
  Zt=diff(log(M1))*100
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_3_IPC_M1.xlsx",overwrite = T)
...
for (i in 0:3) {
  set.seed(525)
  Datos_IPC=read.xlsx("IPC_serie.xlsx",startRow = 13)
  Datos_IPC=Datos_IPC[606:(774+(4*i)),]
  IPC=ts(Datos_IPC$Índice,start = c(2004,12),freq=12)
  Xt=diff(log(IPC),lag=1)*100
  M1=read.csv2("M1.csv",sep=";")
  M1=ts(M1$Miles.de.millones.COP[252:(420+(4*i))],start=c(2004,12),freq=12)
  Zt=diff(log(M1))*100
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_4_IPC_M1.xlsx",overwrite = T)
