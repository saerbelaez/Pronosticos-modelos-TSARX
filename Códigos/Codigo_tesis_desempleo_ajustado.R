...
Datos=Datos[1:168,]
Desempleo=ts(Datos$Desempleo,start = c(2005,1),freq=12)
length(Desempleo)
Xt=diff(Desempleo)
length(Xt)
ISE=ts(Datos$ISE_ajustado,start = c(2005,1),freq=12)
Zt=diff(log(ISE)*100)
par(mfrow=c(1,1))
plot(armasubsets(Xt,nar=40,nma = 0))
####################################### METODO DE CONGDON
set.seed(525)
...
h=25
s=12
step.thv=0.132 #se ajusta el tamaño del paso de acuerdo con 
d0=3
...
#Con d=0, se obtiene que el mejor modelo es el de 2 regímenes
...
Modelo.zt=forecast::Arima(Zt,order=c(8,0,0),include.constant = T,fixed=c(NA,0,0,0,NA,0,NA,NA,NA))
lmtest::coeftest(Modelo.zt)
summary(Modelo.zt)
##################################################### PRONÓSTICOS PARA MODELOS ESTACIONAL MULTIPLICATIVOS 
...
pronosticos<-function(LH, x, thresVar, Iteration, Burnin, step.thv, d0)
{
  ...
  for (i in 1:NN)
  {
    ...
    for (hh in 1:LH)
    {
      zv[nx+hh]=0.3117-0.4242*zv[nx+hh-1]-0.1258*zv[nx+hh-5]+0.1466*zv[nx+hh-7]+0.1553*zv[nx+hh-8]+rnorm(1, mean=0, sd=sqrt(0.513))   ## SE CAMBIA LA FORMULA, el kernel de transici?n, variable umbral
    }
...
### CORRIDA PARA OBTENER PRONÓSTICOS
LH=1
Iteration=20000
Burnin=8000
step.thv<-0.132
...
Comp_des=ts(Comparacion$Desempleo,start = c(2018,12),freq=12)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_1_desempleo_ISE_ajus.xlsx",overwrite = T)
...
for (i in 0:7) {
  set.seed(525)
  Datos=read.xlsx("Desempleo.xlsx")
  Datos=Datos[1:(168+(i*2)),]
  Desempleo=ts(Datos$Desempleo,start = c(2005,1),freq=12)
  Xt=diff(Desempleo)
  Xt=ts(Xt,start=c(2005,2),freq=12)
  ISE=ts(Datos$ISE_ajustado,start = c(2005,1),freq=12)
  Zt=diff(log(ISE),lag=1)*100
  x2=Xt
  thresVar2=Zt
  p=pronosticos(LH, x2, thresVar2, Iteration, Burnin, step.thv, d0)
  px=p$seriex
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_2_desempleo_ISE_ajus.xlsx",overwrite = T)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_3_desempleo_ISE_ajus.xlsx",overwrite = T)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_4_desempleo_ISE_ajus.xlsx",overwrite = T)
