...
pronosticos<-function(LH, x, thresVar, Iteration, Burnin, step.thv, d0)
{
  ...
  for (i in 1:NN)
  {
    ...
    for (hh in 1:LH)
    {
      zv[nx+hh]=0.31165-0.42416*zv[nx+hh-1]-0.1258*zv[nx-hh-5]+0.1258*zv[nx-hh-7]+0.15532*zv[nx-hh-8]+rnorm(1, mean=0, sd=sqrt(0.5131))  ## el kernel de transici?n, variable umbral
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_1_TARdesempleo_ISE_ajus.xlsx",overwrite = T)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_2_TARdesempleo_ISE_ajus.xlsx",overwrite = T)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_3_TARdesempleo_ISE_ajus.xlsx",overwrite = T)
...
for (i in 0:3) {
  ...
  Desempleo=ts(Datos$Desempleo,start = c(2005,1),freq=12)
  Xt=diff(Desempleo)
  Xt=ts(Xt[13:length(Xt)],start=c(2006,2),freq=12)
  ISE=ts(Datos$ISE_estacional,start = c(2005,1),freq=12)
  Zt=diff(log(ISE),lag=1)*100
  x2=Xt
  thresVar2=Zt
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_4_TARdesempleo_ISE_ajus.xlsx",overwrite = T)
...
Modelo_ar=stats::arima(Desempleo,order=c(2,1,0),seasonal = c(2,0,0), include.mean = F)
coeftest(Modelo_ar)
summary(Modelo_ar)
par(mfrow=c(2,1))
...
for (i in 0:14) {
  ...
  Datos=Datos[1:(168+i),]
  Desempleo=ts(Datos$Desempleo,start = c(2005,1),freq=12)
  refit=Arima(Desempleo,order=c(2,1,0),seasonal=c(2,0,0),include.mean = FALSE,model=Modelo_ar)
  AA[i+1,]=forecast(refit,h=1)$mean
}


Pronostico=matrix(NA,nrow=16,ncol=1)
Comparacion=read.xlsx("Desempleo.xlsx")
Pronostico[1,1]=Comparacion[168,4]
Pronostico[2:16,1]=AA[,1]
Pronostico=ts(Pronostico,start=c(2018,12),freq=12)
Pronostico=diff(Pronostico)
...
write.xlsx(data,"Resultados/Pronosticos_1_ARdesempleo_ISE_ajus.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_2_ARdesempleo_ISE_ajus.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_3_ARdesempleo_ISE_ajus.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_4_ARdesempleo_ISE_ajus.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_1_EXPdesempleo_ISE_ajus.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_2_EXPdesempleo_ISE_ajus.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_3_EXPdesempleo_ISE_ajus.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_4_EXPdesempleo_ISE_ajus.xlsx",overwrite = T)
