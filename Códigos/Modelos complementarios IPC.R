...
Modelo.zt=forecast::Arima(Zt,order=c(5,0,0),include.constant = F,fixed=c(NA,0,0,0,NA))
...
pronosticos<-function(LH, x, thresVar, Iteration, Burnin, step.thv, d0)
{
  ...
  for (i in 1:NN)
  {
    ...
    for (hh in 1:LH)
    {
      zv[nx+hh]=-0.1756*zv[nx+hh-1]-0.1417*zv[nx+hh-5]+rnorm(1, mean=0, sd=sqrt(25.01))  ## el kernel de transici?n, variable umbral
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_1_TAR_IPC_M1.xlsx",overwrite = T)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_2_TAR_IPC_M1.xlsx",overwrite = T)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_3_TAR_IPC_M1.xlsx",overwrite = T)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_4_TAR_IPC_M1.xlsx",overwrite = T)
...
Modelo_ar=stats::arima(Xt,order=c(1,0,0),seasonal = c(2,0,0), include.mean = F,fixed=c(NA,NA,NA),transform.pars = F)
coeftest(Modelo_ar)
summary(Modelo_ar)
par(mfrow=c(2,1))
...
Datos_IPC=read.xlsx("IPC_serie.xlsx",startRow = 13)
Pronostico[1:15,1]=as.numeric(AA[,1])
Pronostico=ts(Pronostico,start=c(2019,1),freq=12)
Datos_IPC=read.xlsx("IPC_serie.xlsx",startRow = 13)
Datos_IPC=Datos_IPC[774:789,]
IPC=ts(Datos_IPC$Índice,start = c(2018,12),freq=12)
Comp_des=diff(log(IPC),lag=1)*100
par(mfrow=c(1,1))
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_1_SAR_IPC_M1.xlsx",overwrite = T)
...
legend("topright",                     
       ...
       lty = 1,
       col = 1:2)
write.xlsx(data,"Resultados/Pronosticos_2_SAR_IPC_M1.xlsx",overwrite = T)
...
for (i in 0:4) {
  ...
  IPC=ts(Datos_IPC$Índice,start = c(2004,12),freq=12)
  Xt=diff(log(IPC),lag=1)*100
  refit=Arima(Xt,order=c(1,0,0),seasonal = c(2,0,0), include.mean = F,fixed=c(NA,NA,NA),transform.pars = F,model=Modelo_ar)
...
write.xlsx(data,"Resultados/Pronosticos_3_SAR_IPC_M1.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_4_SAR_IPC_M1.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_1_EXP_IPC_M1.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_2_EXP_IPC_M1.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_3_EXP_IPC_M1.xlsx",overwrite = T)
...
write.xlsx(data,"Resultados/Pronosticos_4_EXP_IPC_M1.xlsx",overwrite = T)
